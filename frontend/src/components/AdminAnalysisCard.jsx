...
export default function AdminAnalysisCard({ result }) {
  const { ats_score, skill_match, education_match, experience_match, missing_skills, education_gap, experience_gap, feedback, provider } = result;
      ...
      <div className="card flex flex-col items-center py-8">
        ...
        {provider && (
          <span className="mt-4 text-xs px-3 py-1 rounded-full flex items-center gap-1.5"
            ...
            {provider === 'groq' ? '⚡' : '🖥️'} via {provider === 'groq' ? 'Groq Cloud' : 'Ollama (Local)'}
          </span>
        )}
      </div>
      ...
      {(skill_match !== undefined || education_match !== undefined || experience_match !== undefined) && (
        <div className="card">
          ...
          <div className="space-y-3">
            {skill_match !== undefined && <ScoreBar label="Skill Match" score={Math.round(skill_match)} />}
            {education_match !== undefined && <ScoreBar label="Education Match" score={Math.round(education_match)} />}
            {experience_match !== undefined && <ScoreBar label="Experience Match" score={Math.round(experience_match)} />}
          </div>
        </div>
      )}
        ...
        <div className="card">
          ...
          <div className="flex flex-wrap gap-2">
            {missing_skills.map((skill, i) => (
              <span key={i} className="px-3 py-1.5 rounded-lg text-xs font-medium" style={{ background: 'rgba(245,158,11,0.1)', color: '#f59e0b', border: '1px solid rgba(245,158,11,0.2)' }}>
                {skill}
              </span>
            ))}
          </div>
